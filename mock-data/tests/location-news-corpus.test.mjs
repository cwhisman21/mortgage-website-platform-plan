...
test("covers every location with four unique articles", () => {
  ...
  assert.equal(index.articles.length, 3144);
  assert.equal(sourceManifest.batches.length, 40);
...
test("keeps the index compact and points to 786 location bundles", () => {
  ...
  assert.equal(contentPaths.size, 786);
...
test("all bundles pass the evidence and anti-filler contract", () => {
  ...
  assert.deepEqual({ articles: result.articles, locations: result.locations }, { articles: 3144, locations: 786 });
...
test("publishes a crawlable static document and sitemap entry for every article", () => {
  ...
  assert.equal(standalonePaths.size, 3144);
